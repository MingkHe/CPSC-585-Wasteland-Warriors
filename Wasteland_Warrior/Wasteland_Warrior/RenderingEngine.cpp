...
void RenderingEngine::updateText() {
	...
	float scale = ((float)game_state->window_width / (float)game_state->window_height) * 0.75f;
		...
		if (game_state->startup) {
			...
			if (game_state->powerText) {
				...
				case 1:
					...
					break;
				case 2:
					pushTextObj(texObjects, "Maximum health increased!", 0.5f*game_state->window_width, 0.8f*game_state->window_height, scale, glm::vec3(0.7f, 0.2f, 0.2f), true);
					break;
				case 3:
